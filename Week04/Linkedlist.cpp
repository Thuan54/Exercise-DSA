...
bool addAfter(NODE* node, int val);
...
bool removeHead(List *&L)
{
    if(!L->p_head) return false;
    if(L->p_tail == L->p_head) L->p_tail = nullptr;
...
void removeBefore(List *&L, int val)
{
    if(!L->p_head || !L->p_head->p_next) return;
...
bool addPos(List *&L, int data, int pos)
{
    ...
    else if(curr && !curr->p_next) addTail(L,data);
...
void removePos(List *&L, int data, int pos)
{
    ...
    else if(curr && !curr->p_next && curr->key == data) removeTail(L);
    else if(curr && curr->p_next && curr->p_next->key == data) removeAfter(curr);
}
...
void removeDuplicate(List *&L)
{
    ...
    while(curr){
        List* newList = new List;
        newList->p_head = curr->p_next;
        newList->p_tail = L->p_tail;
        
        removeElement(newList,curr->key);
        
        L->p_tail = newList->p_tail;
        curr->p_next = newList->p_head;
        curr = curr->p_next;
        delete newList;
    }
    return;
...
bool removeElement(List *&L, int key)
{
    ...
    while (curr)
    {
        if(curr->key == key){
            ...
            if(prev){
                if(curr->p_next) removeAfter(prev);
                else removeTail(L);
            }
            else removeHead(L);
            curr = temp;
